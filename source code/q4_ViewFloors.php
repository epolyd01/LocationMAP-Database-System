...
<body>
	...
	<hr>
  <h2>View Floors in Building with id <?php echo $BID?></h2>
	...
	$tsql=   "Select F.FloorID,F.FName AS [Floor Name],F.Number AS [Floor Number] , F.[Floorplan_File], F.[Small_Description]
FROM FLOOR AS F inner join Building B ON $BID = B.BuildID
order by f.FloorID,f.Number" ;
  ...
  echo("<th>Delete</th>");
	  echo("<th>View Pois</th>");


    $BID = $_GET['GetID'];
echo("<td><a href='q4_InsertFloor.php?Get1=$BID'>Insert Floor</a></td>");//esu to evales
		...
		while ($row = sqlsrv_fetch_array($resultSet, SQLSRV_FETCH_ASSOC)) {
			...
			//esu ta evales
      $id = $row['FloorID'];
          echo("<td><a href='q4_DeleteFloor.php?Get1=$id'>Delete</a></td>");//esu to evales
					  echo("<td><a href='q4_ViewPois.php?Get1=$id'>View Pois</a></td>");//esu to evales
